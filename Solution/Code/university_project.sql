...
/*Условия на защитата на проекта!!!*/

/*Напишете заявка, която извежда имената, авторите и жанровете на 10-те най-популярни песни*/
SELECT song.name AS songName, composer.name AS composerName, genre.name AS genreName
FROM song JOIN song_genre ON song.id = song_genre.song_id
JOIN genre ON song_genre.genre_id = genre.id
...
/Напишете заявка, която извежда имената, авторите и жанровете на 10-те най-продавани песни *imenata, avtorite i janrovete na 10te nai - prodavani pesni*/
 SELECT song.name AS songName,  composer.name AS composerName, genre.name AS genreName
    FROM song 
    JOIN song_genre ON song.id = song_genre.song_id
    JOIN genre ON song_genre.genre_id = genre.id
